...
set_appearance_mode(
    "System"
)  # Setting the appearance mode to follow by the app: "System", "Light" or "Dark"
...
for i in os.listdir(os.getcwd()):
    ...
    os.mkdir(
        "youtube_downloads"
    )  # If there is no folder called "youtube_downloads", create a new one
...
def download_video(entry_field):
    ...
    ):  # If there's an invalid link or empty link, show an error message
        error = CTk()
        error.title("Error")
...
CTkButton(
    master, text="Download", command=lambda *args: download_video(entry.get())
